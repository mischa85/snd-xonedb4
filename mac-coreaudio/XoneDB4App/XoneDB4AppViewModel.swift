...
class XoneDB4AppViewModel: NSObject {
	...
	@Published private var state: XoneDB4AppStateMachine.State = .deactivated
...
extension XoneDB4AppViewModel: OSSystemExtensionRequestDelegate {
	...
	func request(_ request: OSSystemExtensionRequest, foundProperties properties: OSSystemExtensionProperties) {
		if (properties.isEnabled) {
			self.state = XoneDB4AppStateMachine.State.activated
		} else if (!properties.isEnabled) {
			self.state = XoneDB4AppStateMachine.State.deactivated
		}
	}
	...
	func request(_ request: OSSystemExtensionRequest, actionForReplacingExtension existing: OSSystemExtensionProperties, withExtension ext: OSSystemExtensionProperties) -> OSSystemExtensionRequest.ReplacementAction {
		...
		if (self.state == .activating || self.state == .needsActivatingApproval || self.state == .activated || self.state == .activationError) {
			self.state = XoneDB4AppStateMachine.process(self.state, .activationStarted)
		} else if (self.state == .deactivating || self.state == .needsDeactivatingApproval || self.state == .deactivated || self.state == .deactivationError) {
			self.state = XoneDB4AppStateMachine.process(self.state, .deactivationStarted)
		}
		//self.state = XoneDB4AppStateMachine.process(self.state, .activationStarted)
	...
	func request(_ request: OSSystemExtensionRequest, didFinishWithResult result: OSSystemExtensionRequest.Result) {
		...
		if (self.state == .activating || self.state == .needsActivatingApproval || self.state == .activated || self.state == .activationError) {
			...
			self.state = XoneDB4AppStateMachine.process(self.state, .deactivationFinished)
		}
	}
	...
	func request(_ request: OSSystemExtensionRequest, didFailWithError error: Error) {
		...
		if (self.state == .activating || self.state == .needsActivatingApproval || self.state == .activated || self.state == .activationError) {
			...
			self.state = XoneDB4AppStateMachine.process(self.state, .deactivationFailed)
		}
	}
}
#endif
