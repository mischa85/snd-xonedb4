...
static constexpr uint32_t FRAME_RATE = 96000;
static constexpr uint32_t BUFFER_SIZE = 1048576;
static constexpr uint32_t MIDI_RING_SIZE = 256;
static constexpr uint32_t K_PLOYTEC_HARDWARE_SETTLE_MS = 500;
...
kern_return_t IMPL(PloytecDriver, Start)
{
	...
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::Start: failed to start driver: %{public}s", strerror(ret)); return ret; }
	...
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::Start: failed to open device: %{public}s", strerror(ret)); return ret; }
	...
	os_log(OS_LOG_DEFAULT, "PloytecDriver::Start: Device Name: %{public}s", ivars->deviceName.get()->getCStringNoCopy());
	os_log(OS_LOG_DEFAULT, "PloytecDriver::Start: Manufacturer: %{public}s", ivars->manufacturerName.get()->getCStringNoCopy());

	os_log(OS_LOG_DEFAULT, "PloytecDriver::Start: Allocating buffers...");

	ret = CreateBuffers();
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::Start: failed to create buffers: %{public}s", strerror(ret)); return ret; }
	...
	ret = CreateUSBPipes();
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::Start: failed to create USB pipes: %{public}s", strerror(ret)); return ret; }
	...
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::Start: failed to GetDescriptors from PCM in: %{public}s", strerror(ret)); return ret; }
	ret = ivars->usbPCMoutPipe->GetDescriptors(&outDescriptor, kIOUSBGetEndpointDescriptorOriginal);
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::Start: failed to GetDescriptors from PCM out: %{public}s", strerror(ret)); return ret; }
	...
	os_log(OS_LOG_DEFAULT, "PloytecDriver::Start: Transfer mode: %{public}s", ivars->transferMode == BULK ? "BULK" : "INTERRUPT");
	...
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::Start: failed to create USB buffers: %{public}s", strerror(ret)); return ret; }
	...
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::Start: failed to create USB handlers: %{public}s", strerror(ret)); return ret; }

	ivars->usbCurrentUrbCount = 2;
	ret = SendPCMUrbs(ivars->usbCurrentUrbCount);
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::Start: failed to send PCM URBs: %{public}s", strerror(ret)); return ret; }
	ret = SendMIDIUrbs(1);
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::Start: failed to send MIDI URBs: %{public}s", strerror(ret)); return ret; }
	...
	success = ivars->audioDevice->init(this, false, ivars->deviceName.get(), ivars->modelName.get(), ivars->manufacturerName.get(), K_ZERO_TIME_STAMP_PERIOD, inChannelCount, outChannelCount, ivars->usbRXBufferPCM.get(), ivars->usbTXBufferPCMandUART.get(), ivars->transferMode);
	...
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::Start: failed to set name on audio device: %{public}s", strerror(ret)); return ret; }

	ret = AddObject(ivars->audioDevice.get());
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::Start: failed to add object to audio device: %{public}s", strerror(ret)); return ret; }

	ret = RegisterService();
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::Start: failed to register service: %{public}s", strerror(ret)); return ret; }
...
{
	os_log(OS_LOG_DEFAULT, "PloytecDriver::Stop");
	...
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::Stop: super::Stop failed: %{public}s", strerror(ret)); return ret;	}
...
void PloytecDriver::free()
{
	if (ivars != nullptr)
	{
		...
		ivars->usbMIDIinPipe.reset();
		ivars->usbPCMoutPipe.reset();
		ivars->usbPCMinPipe.reset();
		ivars->usbInterface0.reset();
		ivars->usbInterface1.reset();
		ivars->usbPCMinCallback.reset();
		ivars->usbPCMoutCallback.reset();
		ivars->usbMIDIinCallback.reset();
		ivars->midiUserClient = nullptr;
		ivars->midiClient = nullptr;
...
kern_return_t PloytecDriver::NewUserClient_Impl(uint32_t in_type, IOUserClient** out_user_client)
{
	...
	if (in_type == kIOUserAudioDriverUserClientType)
	{
		...
		if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::NewUserClient_Impl: failed to create user client: %{public}s", strerror(ret)); return ret; }
	...
	else
	{
		...
		if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::NewUserClient_Impl: failed to create the PloytecDriver user client: %{public}s", strerror(ret)); return ret; }
...
kern_return_t PloytecDriver::StartDevice(IOUserAudioObjectID in_object_id, IOUserAudioStartStopFlags in_flags)
{
	...
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::StartDevice: failed to start device: %{public}s", strerror(ret)); }
...
kern_return_t PloytecDriver::StopDevice(IOUserAudioObjectID in_object_id, IOUserAudioStartStopFlags in_flags)
{
	...
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::StopDevice: failed to stop device: %{public}s", strerror(ret)); }
...
{
	stats->currentHWFrameRate = ivars->currentHWFrameRate;
...
void PloytecDriver::WriteMIDIBytes(const uint64_t msg) {
	...
	for (uint8_t i = 0; i < length; ++i) {
		...
		uint32_t next = (ivars->midiRingHead + 1) % MIDI_RING_SIZE;
...
bool PloytecDriver::ReadMIDIByte(uint8_t &outByte)
{
	...
	ivars->midiRingTail = (ivars->midiRingTail + 1) % MIDI_RING_SIZE;
	return true;
}
...
kern_return_t PloytecDriver::SendPCMUrbs(uint8_t num)
{
	...
	for (int i = 0; i < num; i++) {
		ret = ivars->usbPCMinPipe->AsyncIO(ivars->usbRXBufferPCMSegment[0].get(), ivars->usbInputPacketSize, ivars->usbPCMinCallback.get(), 0);
		if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::SendPCMUrbs: WriteMIDIBytes: usbPCMinPipe send error: %{public}s", strerror(ret)); return ret; }
		ret = ivars->usbPCMoutPipe->AsyncIO(ivars->usbTXBufferPCMandUARTSegment[0].get(), ivars->usbOutputPacketSize, ivars->usbPCMoutCallback.get(), 0);
		if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::SendPCMUrbs: WriteMIDIBytes: usbPCMoutPipe send error: %{public}s", strerror(ret)); return ret; }
	}
...
kern_return_t PloytecDriver::SendMIDIUrbs(uint8_t num)
{
	...
	ret = ivars->usbMIDIinPipe->AsyncIO(ivars->usbRXBufferMIDI.get(), 512, ivars->usbMIDIinCallback.get(), 0);
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::SendMIDIUrbs: failed to send MIDI URB: %{public}s", strerror(ret)); return ret; }
...
kern_return_t PloytecDriver::AbortUSBUrbs(uint8_t num)
{
	...
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::AbortUSBUrbs: failed to SetConfiguration: %{public}s", strerror(ret)); return ret; }

	IOSleep(10);

	ret = CreateUSBPipes();
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::AbortUSBUrbs: failed to create USB pipes: %{public}s", strerror(ret)); return ret; }

	ret = ivars->usbDevice->DeviceRequest(this, 0x40, 0x49, 0xFFB2, 0x0000, 0x00, nullptr, nullptr, 0);
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::AbortUSBUrbs: failed to flush: %{public}s", strerror(ret)); return ret; }

	ret = SendMIDIUrbs(1);
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::AbortUSBUrbs: failed to send MIDI URBs: %{public}s", strerror(ret)); return ret; }
...
kern_return_t PloytecDriver::CreateUSBTXBuffersPCMandUART(uint32_t outputPacketSize)
{
	...
	for (int i = 0; i < (BUFFER_SIZE / outputPacketSize); i++)
	{
		...
		if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::CreateUSBTXBuffersPCMandUART: failed to create USB output SubMemoryDescriptor: %{public}s", strerror(ret)); return ret; }
...
kern_return_t PloytecDriver::CreateUSBRXBuffersPCM(uint32_t inputPacketSize)
{
	...
	for (int i = 0; i < (BUFFER_SIZE / ivars->usbInputPacketSize); i++)
	{
		...
		if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::CreateUSBRXBuffersPCM: failed to create USB input SubMemoryDescriptor: %{public}s", strerror(ret)); return ret; }
	}
...
kern_return_t PloytecDriver::CreateUSBHandlers()
{
	...
	os_log(OS_LOG_DEFAULT, "PloytecDriver::CreateUSBHandlers: Creating USB handlers...");

	ret = OSAction::Create(this, PloytecDriver_PCMinHandler_ID, IOUSBHostPipe_CompleteAsyncIO_ID, 0, ivars->usbPCMinCallback.attach());
	if (ret != kIOReturnSuccess) return ret;
	ret = OSAction::Create(this, PloytecDriver_PCMoutHandler_ID, IOUSBHostPipe_CompleteAsyncIO_ID, 0, ivars->usbPCMoutCallback.attach());
	if (ret != kIOReturnSuccess) return ret;
	ret = OSAction::Create(this, PloytecDriver_MIDIinHandler_ID, IOUSBHostPipe_CompleteAsyncIO_ID, 0, ivars->usbMIDIinCallback.attach());
	if (ret != kIOReturnSuccess) return ret;
...
kern_return_t PloytecDriver::CreateUSBPipes()
{
	...
	IOUSBHostInterface* usbInterface0 = nullptr;
	IOUSBHostInterface* usbInterface1 = nullptr;
	IOUSBHostPipe* usbMIDIinPipe = nullptr;
	IOUSBHostPipe* usbPCMoutPipe = nullptr;
	IOUSBHostPipe* usbPCMinPipe = nullptr;
	
	os_log(OS_LOG_DEFAULT, "PloytecDriver::CreateUSBPipes: Creating USB pipes...");

	ret = ivars->usbDevice->SetConfiguration(1, false);
	if (ret != kIOReturnSuccess) return ret;
	ret = ivars->usbDevice->CreateInterfaceIterator(&interfaceIterator);
	if (ret != kIOReturnSuccess) return ret;
	ret = ivars->usbDevice->CopyInterface(interfaceIterator, &usbInterface0);
	if (ret != kIOReturnSuccess) return ret;
	if (usbInterface0) ivars->usbInterface0 = OSSharedPtr<IOUSBHostInterface>(usbInterface0, OSNoRetain);
	ret = ivars->usbDevice->CopyInterface(interfaceIterator, &usbInterface1);
	if (ret != kIOReturnSuccess) return ret;
	if (usbInterface1) ivars->usbInterface1 = OSSharedPtr<IOUSBHostInterface>(usbInterface1, OSNoRetain);
	...
	if (ret != kIOReturnSuccess) return ret;
	ret = ivars->usbInterface0->CopyPipe(MIDI_IN_EP, &usbMIDIinPipe);
	if (ret != kIOReturnSuccess) return ret;
	if (usbMIDIinPipe) ivars->usbMIDIinPipe = OSSharedPtr<IOUSBHostPipe>(usbMIDIinPipe, OSNoRetain);
	ret = ivars->usbInterface0->CopyPipe(PCM_OUT_EP, &usbPCMoutPipe);
	if (ret != kIOReturnSuccess) return ret;
	if (usbPCMoutPipe) ivars->usbPCMoutPipe = OSSharedPtr<IOUSBHostPipe>(usbPCMoutPipe, OSNoRetain);
	ret = ivars->usbInterface1->Open(this, 0, NULL);
	if (ret != kIOReturnSuccess) return ret;
	ret = ivars->usbInterface1->SelectAlternateSetting(1);
	if (ret != kIOReturnSuccess) return ret;
	ret = ivars->usbInterface1->CopyPipe(PCM_IN_EP, &usbPCMinPipe);
	if (ret != kIOReturnSuccess) return ret;
	if (usbPCMinPipe) ivars->usbPCMinPipe = OSSharedPtr<IOUSBHostPipe>(usbPCMinPipe, OSNoRetain);
...
kern_return_t PloytecDriver::SetCurrentUrbCount(uint8_t num)
{
	...
	ivars->workQueue->DispatchAsync(^(){
		...
		ret = SendPCMUrbs(num);
		if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::SetCurrentUrbCount: SendPCMUrbs: %{public}s", strerror(ret)); return; }
	});
...
{
	os_log(OS_LOG_DEFAULT, "PloytecDriver::SetFrameCount: Changing input framecount to: %{public}d", inputCount);
	os_log(OS_LOG_DEFAULT, "PloytecDriver::SetFrameCount: Changing output framecount to: %{public}d", outputCount);
...
kern_return_t PloytecDriver::GetChannelCount(uint32_t &inChannelCount, uint32_t &outChannelCount)
{
	...
	os_log(OS_LOG_DEFAULT, "PloytecDriver::GetChannelCount: inputs: %{public}dch outputs: %{public}dch", inChannelCount, outChannelCount);
...
OSSharedPtr<OSString> PloytecDriver::MakeStringFromDescriptor(IOUSBHostDevice* device, uint8_t index)
{
	...
	IOFree(const_cast<IOUSBStringDescriptor*>(desc), length);
...
kern_return_t PloytecDriver::CreateBuffers()
{
	...
	ret = IOBufferMemoryDescriptor::Create(kIOMemoryDirectionInOut, MIDI_RING_SIZE, 0, ivars->MIDIBuffer.attach());
	if (ret != kIOReturnSuccess) return ret;
...
kern_return_t PloytecDriver::GetHardwareStatus()
{
	kern_return_t ret;
	uint16_t bytesTransferred = 0;

	ret = ivars->usbDevice->DeviceRequest(this, 0xC0, static_cast<uint8_t>('I'), 0x0000, 0x0000, 0x01, ivars->usbRXBufferCONTROL.get(), &bytesTransferred, 0);
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::GetHardwareStatus: failed %{public}s", strerror(ret)); return ret; }

	os_log(OS_LOG_DEFAULT, "PloytecDriver::GetHardwareStatus: [0x%{public}02X] %{public}s %{public}s %{public}s %{public}s %{public}s %{public}s",
	       (ivars->usbRXBufferCONTROLAddr[0]),
	       (ivars->usbRXBufferCONTROLAddr[0] & kStatusHighSpeedActive) ? "[HighSpeed]"   : "[FullSpeed]",
	       (ivars->usbRXBufferCONTROLAddr[0] & kStatusLegacyActive)    ? "[Legacy/BCD1]" : "[Modern/BCD3]",
	       (ivars->usbRXBufferCONTROLAddr[0] & kStatusStreamingArmed)  ? "[Armed]"       : "[Disarmed]",
	       (ivars->usbRXBufferCONTROLAddr[0] & kStatusDigitalLock)     ? "[Clock-Lock]"  : "[No-Lock]",
	       (ivars->usbRXBufferCONTROLAddr[0] & kStatusUsbInterfaceOn)  ? "[Streaming]"   : "[Idle]",
	       (ivars->usbRXBufferCONTROLAddr[0] & kStatusSyncReady)       ? "[Stable]"      : "[Syncing]");
...
kern_return_t PloytecDriver::SetHardwareStatus(uint16_t value)
{
	kern_return_t ret;
	uint16_t bytesTransferred = 0;
...
kern_return_t PloytecDriver::GetHardwareFirmwareVersion()
{
	kern_return_t ret = kIOReturnSuccess;
	uint16_t bytesTransferred;
...
kern_return_t PloytecDriver::SetHardwareFrameRate(uint32_t framerate)
{
	kern_return_t ret = kIOReturnSuccess;
	uint16_t bytesTransferred;
	...
	os_log(OS_LOG_DEFAULT, "PloytecDriver::SetHardwareFrameRate: Setting framerate to %d kHz...", framerate);
...
kern_return_t PloytecDriver::GetHardwareFrameRate()
{
	kern_return_t ret = kIOReturnSuccess;
	uint16_t bytesTransferred;
	...
	if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::GetHardwareFrameRate: failed to get current framerate from device: %{public}s", strerror(ret)); return ret; }
	if (bytesTransferred < 3) { os_log(OS_LOG_DEFAULT, "PloytecDriver::GetHardwareFrameRate: framerate: short read (%u bytes)", (unsigned)bytesTransferred); return kIOReturnUnderrun; }
	...
	os_log(OS_LOG_DEFAULT, "PloytecDriver::GetHardwareFrameRate: Got framerate: %u", ivars->currentHWFrameRate);
...
kern_return_t IMPL(PloytecDriver, PCMinHandler)
{
	...
	ivars->workQueue->DispatchAsync(^{
		...
		{
			os_log(OS_LOG_DEFAULT, "PloytecDriver::PCMinHandler: Callback aborted: %{public}s", __func__);
			ret = ivars->usbPCMinPipe->Abort(kIOUSBAbortSynchronous, kIOReturnAborted, NULL);
			if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::PCMinHandler: Abort: %{public}s", strerror(ret)); return; }
			ivars->usbPCMinAbortedCount++;
			return;
		}

		ret = ivars->usbPCMinPipe->AsyncIO(ivars->usbRXBufferPCMSegment[ivars->usbRXBufferPCMCurrentSegment].get(), ivars->usbInputPacketSize, ivars->usbPCMinCallback.get(), 0);
		if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::PCMinHandler: Send: %{public}s", strerror(ret)); return; }
...
kern_return_t IMPL(PloytecDriver, PCMoutHandler)
{
	...
	ivars->workQueue->DispatchAsync(^{
		if (ivars->usbShutdownInProgress)
		{
			os_log(OS_LOG_DEFAULT, "PloytecDriver::PCMoutHandler: Callback aborted: %{public}s", __func__);
			ret = ivars->usbPCMoutPipe->Abort(kIOUSBAbortSynchronous, kIOReturnAborted, NULL);
			if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::PCMoutHandler: Abort: %{public}s", strerror(ret)); return; }
		...
		ret = ivars->usbPCMoutPipe->AsyncIO(ivars->usbTXBufferPCMandUARTSegment[ivars->usbTXBufferPCMandUARTCurrentSegment].get(), ivars->usbOutputPacketSize, ivars->usbPCMoutCallback.get(), 0);
		if (ret != kIOReturnSuccess) { os_log(OS_LOG_DEFAULT, "PloytecDriver::PCMoutHandler: Send: %{public}s", strerror(ret)); return; }
...
kern_return_t IMPL(PloytecDriver, MIDIinHandler)
{
	...
	return ivars->usbMIDIinPipe->AsyncIO(ivars->usbRXBufferMIDI.get(), 512, ivars->usbMIDIinCallback.get(), 0);
}
