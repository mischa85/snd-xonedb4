...
class OzzyViewModel: NSObject {
	...
	@Published private(set) var state: OzzyStateMachine.State = .deactivated
...
extension OzzyViewModel: ObservableObject {

}
	
extension OzzyViewModel {
#if os(macOS)
	func activateMyDext() {
	...
	func activateExtension(_ dextIdentifier: String) {
		...
		self.state = OzzyStateMachine.process(self.state, .activationStarted)
	}
	...
	func deactivateExtension(_ dextIdentifier: String) {
		...
		self.state = OzzyStateMachine.process(self.state, .deactivationStarted)
	}
#endif
}

#if os(macOS)
extension OzzyViewModel: OSSystemExtensionRequestDelegate {
	...
	func requestNeedsUserApproval(_ request: OSSystemExtensionRequest) {
		...
		self.state = OzzyStateMachine.process(self.state, .promptForApproval)
	}

	func request(_ request: OSSystemExtensionRequest, didFinishWithResult result: OSSystemExtensionRequest.Result) {
		os_log("system extension didFinishWithResult: %d", result.rawValue)
		self.state = OzzyStateMachine.process(self.state, .activationFinished)
	}
	...
	func request(_ request: OSSystemExtensionRequest, didFailWithError error: Error) {
		...
		if let extensionError = error as NSError?, extensionError.domain == OSSystemExtensionErrorDomain {
			if extensionError.code == 4 {
				self.state = OzzyStateMachine.process(self.state, .dextNotPresent)
			} else if extensionError.code == 8 {
				self.state = OzzyStateMachine.process(self.state, .codeSigningErr)
			}
		}
		self.state = OzzyStateMachine.process(self.state, .activationFailed)
	}
}
#endif
